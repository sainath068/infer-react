...
const ArticleDerive = ({
  ...
  //     let url = `https://lfaxil11nf.execute-api.ap-south-1.amazonaws.com/source/insights?user_id=${encodeURIComponent(
  //       user.user_id
  //     )}&question=${encodeURIComponent(query)}`;

  //     if (storedSessionId) {
  //       url += `&session_id=${encodeURIComponent(storedSessionId)}`;
  //     }
  ...
  //     // ✅ Debugging: Ensure FormData contains expected values
  //     console.log("FormData contents:");
  //     for (let pair of formData.entries()) {
  //       console.log(`${pair[0]}:`, pair[1]);
  //     }

  //     // ✅ Correct fetch request (NO manual headers)
  ...
  //   }
  // }, [query, storedSessionId, user.user_id, uploadedFile]);
  const handleDeriveClick = useCallback(async () => {
    if (chatHistory && chatHistory.length === 0) {
      if (!uploadedFile) {
        showErrorToast("Please upload a file");
    ...
    try {
      // ✅ Always send question and user_id, optionally session_id
      let url = `https://lfaxil11nf.execute-api.ap-south-1.amazonaws.com/source/insights?user_id=${encodeURIComponent(
        user.user_id
      )}&question=${encodeURIComponent(
        query ? query : "summarize this article"
      )}`;
      ...
      // ✅ FormData is created regardless — file is optional
      const formData = new FormData();
      if (uploadedFile) {
        formData.append("file", uploadedFile);
      }
      const method = storedSessionId ? "PUT" : "POST";
      const fetchOptions = {
        method,
      };

      if (method === "POST") {
        // Always use FormData for POST (file optional)
        fetchOptions.body = formData;
      } else if (method === "PUT") {
        if (uploadedFile) {
          // PUT with file: send FormData and omit Content-Type so browser sets correct multipart type
          fetchOptions.body = formData;
        } else {
          // PUT without file: send empty body and set JSON content-type
  ...
  }, [query, user.user_id, uploadedFile]);
        ...
        style={{
          ...
          border:
            !uploadedFile && chatHistory.length === 0
              ...
              : undefined,
          display:
            !uploadedFile && chatHistory.length === 0 ? "flex" : undefined,
      ...
      >
                  ...
                  >
                    <div
                      ...
                      style={{
                        ...
                        overflow: "auto",
                        overflowX:
                          openNotes || openAnnotate ? "auto" : "hidden",
        ...
        <div
          ...
          style={{
            ...
            width: uploadedFile || chatHistory.length > 0 ? "100%" : "95%",
