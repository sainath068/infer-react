...
const Annotation = ({
  ...
  console.log("Annotation", annotateData);
  ...
  annotateData &&
  typeof annotateData === "object" &&
  annotateData.annotations
    ? Object.entries(annotateData.annotations)
    : [];
    ...
    console.log("sortedAnnotationEntries", sortedAnnotationEntries);
  ...
  .map(([key]) => key) // ONLY the keys (like gene names)
  .join(", ");
      ...
      rows.push(
        <tr className="search-table-body" key={type}>
          ...
          <td style={{ width: "65%" }}>{valueKeys}</td>
        </tr>
      );
